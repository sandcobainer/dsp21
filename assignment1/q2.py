...
def findSnarePosition(snareFilename, drumloopFilename):
    ...
    # find indices of high correlation in loop (unpad the drum_loop)
    corr = corr[len(snare)-1:len(snare)+len(loop)-1]
    return np.where(corr > 0.9)
    


if __name__ == "__main__":
    snareLocations = findSnarePosition('snare.wav', 'drum_loop.wav')
    f = open('results/02-snareLocation.txt','w+')
    f.write(str(snareLocations[0]))
    f.close()
